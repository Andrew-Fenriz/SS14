...
public sealed partial class ElectrocutionSystem : SharedElectrocutionSystem
{
    ...
    private bool IsPowered(EntityUid uid, ElectrifiedComponent electrified, TransformComponent transform)
    {
        ...
        if (electrified.NoWindowInTile)
        {
            ...
            if (tileRef != null)
            {
                if (_entityLookup.GetLocalEntitiesIntersecting(tileRef.Value, flags: LookupFlags.StaticSundries).Any(entity => _tag.HasTag(entity, WindowTag)))
                {
                    return false;
                }
            }
        ...
        DamageSpecifier? shockDamage, TimeSpan time, bool refresh, float siemensCoefficient = 1f)
    {
        ...
        {
            var scaledDamage = shockDamage * siemensCoefficient;
            if (_damageable.TryChangeDamage(uid, scaledDamage, out var damage, origin: sourceUid))
            {
