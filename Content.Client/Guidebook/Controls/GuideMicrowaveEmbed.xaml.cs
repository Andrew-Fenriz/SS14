...
public sealed partial class GuideMicrowaveEmbed : PanelContainer, IDocumentTag, ISearchableControl, IPrototypeRepresentationControl
{
    ...
    private void GenerateSolidIngredients(FoodRecipePrototype recipe)
    {
        ...
        {
            var ingredient = _prototype.Index<EntityPrototype>(product.Id);

            IngredientsGrid.AddChild(new GuideEntityEmbed(product.Id, false, false));
    ...
    private void GenerateLiquidIngredients(FoodRecipePrototype recipe)
    {
        ...
        {
            var reagent = _prototype.Index<ReagentPrototype>(product.Id);
    ...
    private void GenerateMaterialIngredients(FoodRecipePrototype recipe)
    {
        foreach (var (product, amount) in recipe.Ingredients.Materials.OrderByDescending(p => p.Value))
        {
            var material = _prototype.Index<StackPrototype>(product.Id);

            IngredientsGrid.AddChild(new GuideEntityEmbed(material.Spawn, false, false));

            // material name
            var materialNameMsg = new FormattedMessage();
            materialNameMsg.AddMarkupOrThrow(Loc.GetString("guidebook-microwave-material-name-display", ("material", Loc.GetString(material.Name, ("amount", amount)))));
            materialNameMsg.Pop();

            var materialNameLabel = new GuidebookRichPrototypeLink();
            materialNameLabel.SetMessage(materialNameMsg);
            materialNameLabel.LinkedPrototype = material;

            IngredientsGrid.AddChild(materialNameLabel);

            // material quantity
            var materialQuantityMsg = new FormattedMessage();
            materialQuantityMsg.AddMarkupOrThrow(Loc.GetString("guidebook-microwave-material-quantity-display", ("amount", amount)));
            materialQuantityMsg.Pop();

            var materialQuantityLabel = new RichTextLabel();
            materialQuantityLabel.SetMessage(materialQuantityMsg);

            IngredientsGrid.AddChild(materialQuantityLabel);
        }
    }
    ...
    private void GenerateIngredients(FoodRecipePrototype recipe)
    {
        ...
        GenerateMaterialIngredients(recipe);
    }
